...
- **实时对焦追踪（Real-time Focus Tracking）**
...
- **变焦（Zoom）**
  ...
  - 自动模式建议值：在支持光学变焦的设备上，建议限制变焦范围为 `0.5x` 到 `10x`，以避免使用过多的数码变焦而影响图像质量。
  - 使用 `AVCaptureDevice` 的 `videoZoomFactor`

- **镜头畸变校正（Lens Distortion Correction）**
  - 功能：自动或手动校正图像中的几何畸变，尤其是在广角镜头或超广角镜头下，防止图像���缘出现不自然的弯曲。
...
- **焦点微调（Focus Fine-Tuning）**
  - 原因：iOS 原生不支持，需要第三方库实现，且实现复杂度较高。
  - 推荐库：`GPUImage` 或 `Core Image`
