...
@objc public class SCVideoSession: SCSession, AVCaptureFileOutputRecordingDelegate {
    ...
    @objc public init(position: CameraPosition = .back) {
        ...
        defer {
            ...
            do {
                let microphoneInput = try SCSession.captureDeviceInput(type: .microphone)
    ...
    @objc public func setWidth(_ width: Int, height: Int, frameRate: Int) {
        guard
            ...
            let format = SCSession.deviceInputFormat(input: input, width: width, height: height, frameRate: frameRate)
        else {
            return
        }
